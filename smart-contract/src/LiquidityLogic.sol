...
contract LiquidityLogic is ILiquidityLogic {
    ...
    external
    {
        ...
        //emit genesis pool emit event
        emitGenesisPoolInitialized(token, user, tokenAmount, initialDToMint);
    ...
    external
    {
        ...
        //emit pool init event
        emitPoolInitialized(pairId, user, tokenAmount, liquidityTokenAmount);
        ...
        //emit add liquidity stream entered for token
        emitAddLiquidityEntered(0, token, user, tokenAmount, poolAStream.streamCount);

        //emit add liquidity stream entered for liquidityToken
        emitAddLiquidityEntered(0, liquidityToken, user, liquidityTokenAmount, poolBStream.streamCount);
    ...
    external
    {
        ...
        //emit add liquidity stream entered
        emitAddLiquidityEntered(0, liquidityToken, user, liquidityTokenAmount, poolBStream.streamCount);
    ...
    function addOnlyTokenLiquidity(address token, address user, uint256 amount) external {
        ...
        //emit add liquidity stream entered
        emitAddLiquidityEntered(0, token, user, amount, poolAStream.streamCount);
    ...
    function removeLiquidity(address token, address user, uint256 lpUnits) external {
        ...
        //emit liquidity removed stream entered
        emitRemoveLiquidityEntered(0, token, user, lpUnits, streamCount);
