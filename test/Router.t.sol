...
contract RouterTest is Test, Utils {
    ...
    function test_oppositeDirectionSwapExecution_success3() public {
        ...
        */
        ...
        uint256 tokenASwapAmount = 50e18; //
        uint256 tokenBSwapAmount = 18e18; //
        ...
        (Swap[] memory swapsAtoB, uint256 frontAtoB,) = pool.pairStreamQueue(pairIdAtoB);

        uint256 streamsBeforeSwapAtoB = swapsAtoB[frontAtoB].streamsRemaining;

        (, uint256 amountOutSwapA) = poolLogic.getSwapAmountOut(
            swapsAtoB[frontAtoB].swapAmountRemaining, reserveAA, reserveAB, reserveDA, reserveDB
        );

        (, uint256 amountOutSwapB) =
            poolLogic.getSwapAmountOut(tokenBSwapAmount, reserveAB, reserveAA, reserveDB, reserveDA);

        console.log("B -> A %s", streamsBeforeSwapBtoA);
        console.log("A -> B %s", streamsBeforeSwapAtoB);

        console.log("A -> B REMAINING %s", swapsAtoB[frontAtoB].swapAmountRemaining);
        console.log("A -> B AMOUNT OUT %s", swapsAtoB[frontAtoB].amountOut);

        console.log("Amount Out A %s", amountOutSwapA);
        console.log("Amount Out B %s", amountOutSwapB);
        ...
        console.log("AMOUNT REMNG B-A %s", swapsBtoA[frontBtoA].swapAmountRemaining);

        assertEq(user2TokenABalanceAfter, amountOutSwapB); 
