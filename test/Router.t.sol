...
contract RouterTest is Test, Utils {
    ...
    function test_oppositeDirectionSwapExecution_success() public {
        ...
        */
        ...
        uint256 tokenASwapAmount = 30e18; //4 streams
        uint256 tokenBSwapAmount = 18e18; //1 stream
...
console.log(swapAtoB.swapAmountRemaining,"swapAtoB.swapAmountRemaining is me");
console.log(SLIPPAGE,"SLIPPAGE is me");
console.log(dToPassAgain,"dToPassAgain is me");
console.log(swapAtoB.swapPerStream,"swap per stram");
console.log(swapAtoB.streamsCount,"stream count");
console.log(reserveDAa,"reserveDAa");
console.log(reserveDBb,"reserveDBb");
        ...
        console.log("AMMMOUNTTT %s",swapsBtoA[frontBtoA - 1].amountOut); // should return 0.
    ...
    // }
      ...
      function test_oppositeDirectionSwapExecution_success3() public {
        ...
        */
        ...
        uint256 tokenASwapAmount = 50e18; //5 streams
        uint256 tokenBSwapAmount = 18e18; //1 stream
        ...
        (, uint256 swapAmountOutBtoABeforeSwap) =
            poolLogic.getSwapAmountOut(tokenBSwapAmount, reserveAB, reserveAA, reserveDB, reserveDA);
        ...
        // // get swap from queue
        (Swap[] memory swapsAtoBAfterSwap, uint256 frontAtoBa, uint256 backAtoBa) = pool.pairStreamQueue(pairIdAtoB);
        Swap memory swapAtoB = swapsAtoBAfterSwap[frontAtoBa ]; // as A - B is consumed fully
        
        console.log(swapsAtoB[frontAtoB].amountOut,"swapsAtoB[frontAtoB].amountOut before");
        console.log(swapAtoB.completed,"swapAtoB.completed");
        console.log(swapAtoB.amountOut,"swapAtoB.amountOut");
        console.log(swapAtoB.swapAmountRemaining,"swapAtoB.swapAmountRemaining");
        (uint256 reserveDAa,, uint256 reserveAAa,,,,,) = pool.poolInfo(address(tokenA));
        ...
        (Swap[] memory swapsBtoA, uint256 frontBtoA, uint256 backBtoA) = pool.pairStreamQueue(pairIdBtoA);
        assertEq(frontBtoA, backBtoA); // should be consumed 
        assertEq(swapsBtoA[frontBtoA -1 ].completed, true);

        console.log("AMMMOUNTTT %s",swapsBtoA[frontBtoA - 1].swapAmountRemaining); // should return 0.
        console.log("AMMMOUNTTT OUT %s",swapsBtoA[frontBtoA - 1].amountOut);
        assertEq(user2TokenABalanceAfter, swapAmountOutBtoABeforeSwap); 
