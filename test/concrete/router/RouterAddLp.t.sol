...
contract RouterTest is Deploys {
    ...
    function test_addLiqDualToken_success() public {
        ...
        (uint256 reserveDBeforeA,uint256 poolOwnershipUnitsTotalBeforeA, uint256 reserveABeforeA,,,) = pool.poolInfo(address(tokenA));
        ...
        (uint256 dToTransfer,) = poolLogic.getSwapAmountOut(swapPerStreamDToToken, reserveABeforeA, 0, reserveDBeforeA, 0);

        uint256 lpUnitsBeforeFromToken = poolLogic.calculateLpUnitsToMint(poolOwnershipUnitsTotalBeforeB, swapPerStreamInputToken, swapPerStreamInputToken+reserveABeforeB, 0, reserveDBeforeB);
        uint256 lpUnitsBeforeFromD = poolLogic.calculateLpUnitsToMint(lpUnitsBeforeFromToken + poolOwnershipUnitsTotalBeforeB, 0, swapPerStreamInputToken+reserveABeforeB, dToTransfer, reserveDBeforeB+dToTransfer);
        ...
        assertEq(streamsAfterDual[frontAD].poolBStream.swapAmountRemaining, dToTokenAmountSingle - swapPerStreamDToToken);
    ...
    function test_addToPoolSingle_success() public {
        ...
        (LiquidityStream[] memory streamsAfterDual, uint256 frontAD, uint256 backAD) = pool.liquidityStreamQueue(pairIdSingle);
    ...
    function test_streamDToPool_success() public {
        ...
        uint256 lpUnitsBeforeFromD = poolLogic.calculateLpUnitsToMint(poolOwnershipUnitsTotalBeforeB, 0, reserveABeforeB, dToTransfer, reserveDBeforeB+dToTransfer);
