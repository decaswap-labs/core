...
contract RouterTest_Swap is RouterTest {
    ...
    function test_swapMarketOrder_invalidExecutionPrice() public {
        // Attempt to swap with an invalid execution price (e.g., zero)
        vm.prank(owner);
        vm.expectRevert(IRouterErrors.InvalidExecutionPrice.selector);
        router.swapMarketOrder(address(tokenA), address(tokenB), 10 ether);
    }
