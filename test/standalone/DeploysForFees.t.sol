...
contract DeploysForFees is Test {
    ...
    function _createPoolLogic() internal ownerAction {
        poolLogic = new PoolLogic(owner, address(0), address(333));
    }

    function _createPool() internal ownerAction {
        pool = new Pool(address(0), address(router), address(poolLogic), address(333));
    }
    ...
    function _createFeesContract() internal ownerAction {
        feesLogic = new FeesLogic(address(pool), address(poolLogic));
    }
